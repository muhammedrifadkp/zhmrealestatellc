...
export default function AlReemIslandPage() {
  ...
  // Filter projects for Al Reem Island
  const filteredProjects = useMemo(() => {
    return projectsData.filter(project => 
      project.location.toLowerCase().includes("al reem island")
    );
  }, []);
    ...
    <main className="min-h-screen bg-[#f8f9fb] pt-32 pb-20">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="container mx-auto px-4 py-4">
          <nav className="flex text-xs text-gray-400 gap-2 items-center">
            ...
            <span>/</span>
            <Link href="/projects/abu-dhabi" className="hover:text-primary transition-colors">Abu Dhabi</Link>
            <span>/</span>
            <span className="text-gray-600">Al Reem Island</span>
          </nav>
        </div>
      </div>
      ...
      <div className="bg-white border-b border-gray-100 mb-8">
        <div className="container mx-auto px-4 py-12">
          ...
          >
            Real Estate Projects in Al Reem Island, Abu Dhabi
          </h1>
      ...
      <div className="container mx-auto px-4">
            ...
            >
              ...
              <Link href={`/projects/${project.slug}`} className="relative h-64 overflow-hidden block">
                ...
                <div className="absolute top-4 left-4">
                  <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg ${project.features.handover === 'Completed' ? 'bg-secondary text-white' : 'bg-primary text-black'}`}>
                    {project.features.handover === 'Completed' ? 'COMPLETED' : 'OFF-PLAN'}
                  </span>
                </div>
                {/* Logo Placeholder like image */}
              ...
              <div className="p-6 flex flex-col flex-grow">
                ...
                <div className="grid grid-cols-3 gap-2 mt-auto py-4 border-t border-gray-50">
                  <div className="flex flex-col items-center gap-1.5">
                    ...
                    <span className="text-[10px] text-gray-500 font-bold uppercase">{project.features.propertyType}</span>
                  </div>
                  <div className="flex flex-col items-center gap-1.5">
                    <BedDouble size={14} className="text-gray-300" />
                    <span className="text-[10px] text-gray-500 font-bold uppercase">{project.features.bedrooms.split(' ')[0]} BR</span>
                  </div>
                  <div className="flex flex-col items-center gap-1.5">
                    <Calendar size={14} className="text-gray-300" />
                    <span className="text-[10px] text-gray-500 font-bold uppercase">{project.features.handover.split(' ')[0]} {project.features.handover.split(' ')[1] || ''}</span>
                  </div>
                </div>
              </div>
            </motion.div>
        ...
        {filteredProjects.length === 0 && (
          <div className="py-20 text-center">
            ...
            <p className="text-gray-500">We couldn't find any projects in Al Reem Island at the moment.</p>
          </div>
        )}
