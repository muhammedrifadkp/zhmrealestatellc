...
export default function SaadiyatIslandPage() {
  ...
  // Filter projects for Saadiyat Island
  const filteredProjects = useMemo(() => {
    return projectsData.filter(project => 
      project.location.toLowerCase().includes("saadiyat island")
    );
  }, []);
    ...
    <main className="min-h-screen bg-[#f8f9fb] pt-32 pb-20">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="container mx-auto px-4 py-4">
          <nav className="flex text-xs text-gray-400 gap-2 items-center">
            ...
            <span>/</span>
            <span className="text-gray-600">Saadiyat Island</span>
          </nav>
        </div>
      </div>
      ...
      <div className="bg-white border-b border-gray-100 mb-8">
        <div className="container mx-auto px-4 py-12">
          ...
          >
            Real Estate Projects in Saadiyat Island, Abu Dhabi
          </h1>
      ...
      <div className="container mx-auto px-4">
            ...
            >
              ...
              <Link href={`/projects/${project.slug}`} className="relative h-64 overflow-hidden block">
                ...
                <div className="absolute top-4 left-4">
                  <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg ${project.features.handover === 'Completed' || project.features.paymentPlan === 'Completed' ? 'bg-secondary text-white' : 'bg-primary text-black'}`}>
                    {project.features.handover === 'Completed' || project.features.paymentPlan === 'Completed' ? 'COMPLETED' : 'OFF-PLAN'}
                  </span>
                </div>
                ...
                <div className="grid grid-cols-3 gap-2 mt-auto py-4 border-t border-gray-50">
                  ...
                  <div className="flex flex-col items-center gap-1.5">
                    ...
                    <span className="text-[10px] text-gray-500 font-bold uppercase text-center">
                       {project.features.handover === 'Completed' ? 'READY' : project.features.handover.split(' ')[0]} {project.features.handover !== 'Completed' ? project.features.handover.split(' ')[1] || '' : ''}
                    </span>
                  </div>
                </div>
              </div>
        ...
        {filteredProjects.length === 0 && (
          <div className="py-20 text-center">
            ...
            <p className="text-gray-500">We couldn't find any projects in Saadiyat Island at the moment.</p>
          </div>
        )}
