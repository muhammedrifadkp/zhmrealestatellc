...
export default function TownSquareDubaiPage() {
  const [viewMode, setViewMode] = useState<'gallery' | 'map' | 'list'>('gallery');
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 12;

  // Filter projects for Town Square Dubai
  const filteredProjects = useMemo(() => {
    return projectsData.filter(project => 
      project.location.toLowerCase().includes("town square dubai")
    );
  }, []);
    ...
    <main className="min-h-screen bg-[#f8f9fb] pt-32 pb-20">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="container mx-auto px-4 py-4">
          <nav className="flex text-xs text-gray-400 gap-2 items-center">
            ...
            <span>/</span>
            <span className="text-gray-600">Town Square Dubai</span>
          </nav>
        </div>
      </div>
      ...
      <div className="bg-white border-b border-gray-100 mb-8">
        <div className="container mx-auto px-4 py-12">
          ...
          >
            Real Estate Projects in Town Square Dubai, Dubai
          </h1>
        ...
        {filteredProjects.length === 0 && (
          <div className="py-20 text-center">
            <div className="center-flex flex flex-col items-center">
              ...
              <p className="text-gray-500 text-center">We couldn't find any projects in Town Square Dubai at the moment.</p>
            </div>
          </div>
        )}
